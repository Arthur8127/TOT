...
public class MatchController : NetworkBehaviour
{
    ...
    public void OnPlayerDisconnected(NetworkConnection conn)
    {
        ...
        else
        {
            ...
            if (playeCount < 3)
            {
                StartCoroutine(ResultVotingWhite("Один из мирных покиул игру, предатель победил", MatchState.End, true));
                return;
            }
    ...
    public void AnalisVoiting()
    {
        ...
        {
            msg = "Не один кондидат не набрал мин. голосов";
    ...
    private void CheckWinner(Player player)
    {
        if (player.isRaider)
        {
            StartCoroutine(ResultVotingWhite("Выкинули предателя по имени: " + player.NickName, MatchState.End, false));
        }
        else
        {
            // выкинули мирного
            if (GetLivePlayers() > 2)
            {
                ...
                StartCoroutine(ResultVotingWhite("Избавились от мирного по имени: " + player.NickName, MatchState.GamePlay, false));
            }
            else
            {
                // предатель победил надо заканчивать
                StartCoroutine(ResultVotingWhite("Избавились от мирного по имени: " + player.NickName, MatchState.End, true));
            }
        }
    }
